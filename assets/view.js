  ...
  const slug = document.getElementById("slug-text").innerText;
...
function loadContent() {
  ...
  if(screen === "small") {
    window.location.href = `../../viewer-s/${value}/${slug}`;
  }
...
function getSigninStatus() {
  ...
  const user = document.getElementById("who");
...
if(ext === ".pdf") {
viewer_html = `<div id="pdf-parent" class="pdf-parent"><div><button id="max_btn">🗖 Maximize</button>`;
viewer_html += `<button id="min_btn">🗕 Minimize</button></div>`;
viewer_html += `<iframe id="docViewer" class="doc-viewer pdf-frame" src="/web/viewer.html?file=${path}" frameborder="0" oncontextmenu="return false"></iframe></div>`;
}
...
side_nav.classList.toggle("active");
  ...
  else if (max_btn && event.target === max_btn) {
    maximizePdf();
  }
  else if (min_btn && event.target === min_btn) {
    minimizePdf();
  }
  ...
  else if(pickerVisible && event.target !== emojiBtn && event.target !== pickerContainer && event.target !== picker) {
